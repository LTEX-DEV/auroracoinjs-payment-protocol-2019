...
var ECDSA = auroracoin.crypto.ECDSA;
var sha256sha256 = auroracoin.crypto.Hash.sha256sha256;
var varintBufNum = auroracoin.encoding.BufferWriter.varintBufNum;

// BIP 70 - payment protocol
function PaymentProtocol(currency) {
  this.messageType = null;
  this.message = null;
  this.currency = currency || 'AUR';
}
...
PaymentProtocol.LEGACY_PAYMENT.REQUEST_CONTENT_TYPE = 'application/auroracoin-paymentrequest';
PaymentProtocol.LEGACY_PAYMENT.CONTENT_TYPE = 'application/auroracoin-payment';
PaymentProtocol.LEGACY_PAYMENT.ACK_CONTENT_TYPE = 'application/auroracoin-paymentack';
...
PaymentProtocol.prototype.makePaymentRequest = function(obj, currency) {
  ...
  this.currency = this.currency || currency || 'AUR';
  this.setObj(obj);
  return this;
};
...
PaymentProtocol.prototype.getContentType = function() {
  if (this.messageType === 'PaymentRequest') {
    return PaymentProtocol.LEGACY_PAYMENT.REQUEST_CONTENT_TYPE;
  }

  if (this.messageType === 'Payment') {
    return PaymentProtocol.LEGACY_PAYMENT.CONTENT_TYPE;
  }

  if (this.messageType === 'PaymentACK') {
    return PaymentProtocol.LEGACY_PAYMENT.ACK_CONTENT_TYPE;
  }
...
function magicHash(str) {
  var magicBytes = new Buffer('Auroracoin Signed Message:\n');
